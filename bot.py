...
def remove_city_from_title(title: str) -> str:
    for city_key in KZ_CITIES.keys():
        # 🔥 НОВОЕ: Убиваем город в самом начале (даже если перед ним эмодзи, пробелы или знаки)
        title = re.sub(rf"^[\s\W]*{city_key}\b", "", title, flags=re.IGNORECASE)
        # Убиваем город внутри текста
        title = re.sub(rf"\b{city_key}\b", "", title, flags=re.IGNORECASE)
        
    title = re.sub(r"\s{2,}", " ", title)
    # 🔥 Жестко вычищаем повисшие знаки препинания в начале строки
    title = re.sub(r"^[\s,\-•:!]+", "", title) 
    return title.strip(" -–•:,!")

def fix_glued_words(text: str) -> str:
    # 🔥 НОВОЕ: Отлепляем буквы от знаков препинания ("100 000!в" -> "100 000! в")
    text = re.sub(r'([!?,.])([А-Яа-яЁёA-Za-z])', r'\1 \2', text)
    
    # Расклеиваем языки и регистры ("вQostanai" -> "в Qostanai", "Hubпройдет" -> "Hub пройдет")
