...
def remove_city_from_title(title: str) -> str:
    # Сортируем города от длинных к коротким, чтобы не удалять части слов
    sorted_cities = sorted(KZ_CITIES.keys(), key=len, reverse=True)
    
    for city_key in sorted_cities:
        # Удаляем город в начале строки или как отдельное слово
        pattern = re.compile(rf"(?i)(^|[\s\W]){city_key}(\b|[\s\W]|$)", re.IGNORECASE)
        title = pattern.sub(r"\1", title)

    # Чистим заголовок от мусора после удаления
    title = re.sub(r"\s{2,}", " ", title)
    # Убираем повисшие в начале знаки и предлоги (исправит "!в в Костанай")
    title = re.sub(r"^[,\-•:!\s]+", "", title)
    title = re.sub(r"^(в|во|на|at|in)\s+", "", title, flags=re.IGNORECASE)
    
    return title.strip(" -–•,:!")

def fix_glued_words(text: str) -> str:
    # Разделяем знаки препинания и буквы (исправит "$100 000!в")
    text = re.sub(r'([!?,.])([А-Яа-яЁёA-Za-z])', r'\1 \2', text)
    
    # Разделяем кириллицу и латиницу (исправит "вQostanai" и "Hubпройдет")
    text = re.sub(r'([а-яёА-ЯЁ])([A-Za-z])', r'\1 \2', text)
    text = re.sub(r'([A-Za-z])([а-яёА-ЯЁ])', r'\1 \2', text)
    
    # Стандартная расклейка регистров
    text = re.sub(r'([а-яё])([А-ЯЁ])', r'\1 \2', text)
    
    # Убираем двойные предлоги ("в в Qostanai" -> "в Qostanai")
